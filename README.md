...
| H3-to-h2c backpressure and Priority | **Fork-only, corrected after field deadlock and lossy-path starvation** | ~~256/128 KiB connection queue watermarks were tracked per stream and paused the shared h2c socket.~~ **Replaced:** the watermarks now form one state machine per downstream QUIC connection, while the shared h2c fd remains readable. nghttp2 connection credit is returned after copying into the bounded response buffer, but stream credit is withheld until xquic accepts those bytes, isolating a pause to that response. ~~Low-water recovery walked the stream list from its head, so a large response could refill the connection queue before later module chunks ran.~~ **Replaced:** each connection retains a round-robin cursor and admits at most one 16 KiB body chunk per stream per pass; a stream that observes high water without sending owns the next recovery turn. Buffered responses also clamp the server event-loop wake to 10 ms, so progress does not depend on xquic delivering a write notification. RFC 9218 `Priority` mapping and xquic's urgent packet queue remain. Multi-stream high/low-water regression coverage is in `tests/test_h2_proxy.c`. |
