...
| H3-to-h2c backpressure and Priority | **Fork-only, corrected after field deadlock** | ~~256/128 KiB connection queue watermarks were tracked per stream and paused the shared h2c socket.~~ **Replaced:** the watermarks now form one state machine per downstream QUIC connection, while the shared h2c fd remains readable. nghttp2 connection credit is returned after copying into the bounded response buffer, but stream credit is withheld until xquic accepts those bytes, isolating a pause to that response. Every server tick probes low water and retries parked streams, so notification loss cannot deadlock recovery; `EAGAIN` remains a per-stream signal. RFC 9218 `Priority` mapping and xquic's urgent packet queue remain. Regression coverage is in `tests/test_h2_proxy.c`. |
