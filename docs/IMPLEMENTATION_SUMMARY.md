...
2. Linux protocol regression：RFC v1/v2 fixed vector、
   SNI exact/wildcard、乱序 ClientHello、双向 UDP fallback、v1/v2 Retry DCID
   切换、伪造 Retry、Retry 状态超时、真实 nghttp2 双向代理、config parity、
   server 和 CONNECT-IP/TCP egress 回归。
3. Linux ASan/UBSan：SNI 和 H2 协议测试，包含 H3 提前关闭后的 nghttp2 生命周期。

所有初始化 xquic 子模块的 Actions 都先执行 `scripts/apply_xquic_patches.sh`；xquic
缓存键包含 `patches/xquic/*.patch` 的内容哈希，防止补丁变化后复用旧二进制。
