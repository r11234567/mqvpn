...
mqvpn_version_string(void)
{
    ...
    if (buf[0] == '\0') {
        ...
        snprintf(buf, sizeof(buf), "%d.%d.%d.%d%s", MQVPN_VERSION_MAJOR,
                 MQVPN_VERSION_MINOR, MQVPN_VERSION_PATCH, MQVPN_VERSION_TWEAK,
                 MQVPN_VERSION_SUFFIX_STR);
#else
        snprintf(buf, sizeof(buf), "%d.%d.%d.%d", MQVPN_VERSION_MAJOR,
                 MQVPN_VERSION_MINOR, MQVPN_VERSION_PATCH, MQVPN_VERSION_TWEAK);
#endif
    }
    return buf;
