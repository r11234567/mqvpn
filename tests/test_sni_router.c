...
test_retry_dcid_routing(const char *initial_hex, const char *retry_hex, int forge_tag)
{
    ...
    uint8_t retry[128];
    size_t retry_len = decode_hex(retry_hex, retry, sizeof(retry));
    size_t retry_scid_len_offset = 6 + retry[5];
    size_t retry_scid_len = retry[retry_scid_len_offset];
    const uint8_t *retry_scid = retry + retry_scid_len_offset + 1;
    assert(retry_scid_len == packet[5]);
...
{
    static const char rfc9001_retry[] =
        "ff000000010008f067a5502a4262b5746f6b656e04a265ba2eff4d829058fb3f0f2496ba";
    static const char rfc9369_retry[] =
        "cf6b3343cf0008f067a5502a4262b5746f6b656ec8646ce8bfe33952d955543665dcc7b6";

    test_retry_dcid_routing(RFC9001_CLIENT_INITIAL, rfc9001_retry, 0);
    test_retry_dcid_routing(RFC9369_CLIENT_INITIAL, rfc9369_retry, 0);
    test_retry_dcid_routing(RFC9001_CLIENT_INITIAL, rfc9001_retry, 1);
}
